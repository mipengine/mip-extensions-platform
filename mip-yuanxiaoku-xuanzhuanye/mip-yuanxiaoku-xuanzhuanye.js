/**
 * @file mip-yuanxiaoku-xuanzhuanye 组件
 * @author
 */
...
define(function (require) {
    ...
    customElement.prototype.firstInviewCallback = function () {
        ...
        .then(function (res) {
            ...
            $element.find('.select-zhuanye-dalei ul')
            .html(
                list.map(function (item, index) {
                    return (
                        '<li data-big-class-id=\"' + item.ID + '\">'
                        +    item.Name
                        + '</li>'
                    );
                })
            ...
            $element.find('.select-zhuanye-dalei ul li:nth-of-type(1)').addClass('current');
            bigClassId = $element.find('.select-zhuanye-dalei ul li:nth-of-type(1)').data('big-class-id');

            loadSmallClass();
        });
        $element.find('.select-zhuanye-dalei').on('click', '.open-close', function () {
            $element.find('.select-zhuanye-dalei ul li:nth-of-type(n+7)').toggle();
            $(this).toggleClass('active');
        });
        // 专业小类
        function loadSmallClass() {
            $element.find('.zhuanye-list .load-info .loading').show();
            fetch('https://data.api.ppkao.com/Interface/YXK/SchoolSpecialtyApi.ashx?action=GetProfessionalSmall&zytypeid=' + bigClassId)
            .then(function (res) {
                return res.json();
            }).then(function (json) {
                // console.log(decode(json));
                ...
                $element.find('.zhuanye-list .list-container ul')
                .html(
                    list.map(function (item, index) {
                        return (
                            '<li>'
                            +    '<h2>'
                            +        '<b>' + item.specialname + '</b><span></span>'
                            +    '</h2>'
                            +    '<div class=\"details\">'
                            +        '<article>'
                            +            '<span>'
                            +                '专业大类：<b>' + item.zytype + '</b>'
                            +            '</span>'
                            +            '<span>'
                            +                '专业小类：<b>' + item.specialname + '</b>'
                            +            '</span>'
                            +            '<span>'
                            +                '专业代码：<b>' + item.code + '</b>'
                            +            '</span>'
                            +        '</article>'
                            +        '<aside>'
                            +            '<a class=\"red\" href=\"zhuanyedetails.html?Id=' + item.ID
                                            + '&name=' + base64('encode', item.specialname)
                                            + '&category=details\">专业详情</a>'
                            +            '<a class=\"blue\" href=\"zhuanyedetails.html?Id=' + item.ID
                                            + '&name=' + base64('encode', item.specialname)
                                            + '&category=college\">开设院校</a>'
                            +        '</aside>'
                            +    '</div>'
                            + '</li>'
                        );
                ...
                $element.find('.zhuanye-list .load-info .loading').hide();
            });
        }

        $element.find('.select-zhuanye-dalei').on('click', 'ul li', function () {
            if (!$(this).hasClass('current')) {
                $(this).addClass('current').siblings('li').removeClass('current');

                bigClassId = $(this).data('big-class-id');
                $element.find('.zhuanye-list .list-container ul').html('');
                $element.find('.zhuanye-list .load-info .loading').show();
                loadSmallClass();
            }
        });

        $element.find('.zhuanye-list .list-container').on('click', 'ul li h2', function () {
            $(this).find('span').toggleClass('active');
            $(this).parents('li').find('.details').toggle();
        });
        $element.find('.zhuanye-list .list-container').on('click', 'ul li .details aside .red', function () {
            customStorage.set('xuanZhuanYeCategory', 'details');
        });

        // 搜索
        $element.find('.search-zhuanye input').val(
            (customStorage.get('searchPre') ? customStorage.get('searchPre') : '')
        );
        ...
        $element.find('.search-zhuanye').on('click', '.button', function () {
            loadInput();
        });
        function loadInput() {
            var text = $element.find('#search-input').val();
            customStorage.set('searchPre', text);
            if (text === '') {
                alert('请输入关键字！');
                return;
            } else {
                zhuanYeSearch(text);
                $element.find('.search-result').addClass('active');
            }
        }
        $element.find('.search-result').on('click', '.quxiao', function () {
            $element.find('.search-result').removeClass('active');
        });
        function zhuanYeSearch(val) {
            $element.find('.search-result .content ul').html('');
            $element.find('.search-result .content .load-info .none').hide();
            $element.find('.search-result .content .load-info .loading').show();
            var text = urlEncode(val);
            // console.log(text);
            fetch('https://data.api.ppkao.com//Interface/YXK/SchoolSpecialtyApi.ashx/?action=SelectProfessional&specialname=' + text)
            .then(function (res) {
                return res.json();
            }).then(function (json) {
                // console.log(decode(json));
                if (decode(json).S !== '0') {
                    ...
                    $element.find('.search-result .content ul').html(
                        list.map(function (item, index) {
                            return (
                                '<li>'
                                +    '<a href=\"zhuanyedetails.html?Id=' + item.ID
                                +        '&name=' + base64('encode', item.specialname) + '&category=details\">'
                                +        '<b>' + item.zytype + '</b>：<span>' + item.specialname + '</span>'
                                +    '</a>'
                                + '</li>'
                            );
                        })
                    );
                    $element.find('.search-result .content .load-info .loading').hide();
                    $element.find('.search-result .content .load-info .none').show();
                } else {
                    $element.find('.search-result .content .load-info .loading').hide();
                    $element.find('.search-result .content .zanwu-shuju').show();
                }
            });
        }

        // 热门搜索
        $element.find('.search-zhuanye').on('click', '.hot-search a', function () {
            customStorage.set('xuanZhuanYeCategory', 'details');
            zhuanYeSearch($(this).text());
            $element.find('.search-result').addClass('active');
        });
