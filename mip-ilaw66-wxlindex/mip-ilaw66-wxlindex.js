...
define(function (require) {
    ...
    customElement.prototype.firstInviewCallback = function () {
        ...
        $el.find('#orderlist').click(function () {
            window.top.location.href = 'orderlist?channel=' + channel + '&frompage=index';
        });

        $el.find('#account').click(function () {
            window.top.location.href = 'lawyerlist?channel=' + channel;
        });
        ...
        //      var url = window.window.top.location.href;
        //      window.history.replaceState({page: 'back'}, null, backurl);
        //      window.history.pushState({page:'now'}, null, url);
        //      // IOS9及以上第一层页面不会立即触发,需要页面上有交互。
        //      window.onpopstate = function () {
        //          history.state.page && 'back' === history.state.page && window.location.reload()
        //      };
        //  };
        //  goback('./');
        //  公共的
        ...
        $el.find('.media').on({
            touchstart: function (e) {
                ...
                $el.find(this).attr('class', 'media').css('background', 'rgba(218,209,204, 1)');
            }
        });
        ...
        $el.find('.radio-rule').on('click', function () {
            ...
            $el.find(this).hasClass('rule-checked')
                ? $el.find(this).removeClass('rule-checked') : $el.find(this).addClass('rule-checked');
        ...
        $el.find('.tab-content__close').on('click', function () {
            $el.find(this).parent().parent().removeClass().addClass('tab-pane');
            flg = 0;
        });
        $el.find('.media').on('click', function (event) {
            console.log($el.find(this).data('href'));
            tabHref = $el.find(this).data('href');
            $el.find('#' + $el.find(this).data('href')).removeClass().addClass('tab-pane active');
            flg = 1;
        ...
        $el.find('.media').click(function () {
            questionType = $el.find(this).data('type');
            // 百度统计
        ...
        $el.find('.media').click(function () {
            questionType = $el.find(this).data('type');
            var num = $el.find(this).data('num');
            // 百度统计
            // window._hmt && window._hmt.push(['_trackEvent', questionType, 'click']);
            // window._hmt && window._hmt.push(['_trackEvent', questionType + '_' + $el.find('#channel').val() + '_' + num, 'click']);
        });
        // 触屏时底色变更
        var timeOutEvent = 0;
        $el.find('.media').on({
        ...
        $el.find('.consulting').on('click', function () {
            if (!$el.find(this).hasClass('noChecked')) {
                var questionType = $el.find(this).data('type');
                directOrOrder(questionType);
            //  window.top.location.href = 'request?questionType=' + questionType;
            }
            else if ($el.find(this).hasClass('noChecked')
