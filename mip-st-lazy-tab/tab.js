...
define(function () {
    ...
    var _init = function(opt) {
        ...
        _setWrap = function ($wrapper) {
            var _this = this;
            $wrapper.children().eq(0).wrap('<div class="mip-st-lazy-tab-scroll-touch"></div>');
            ...
            if ($wrapper.children().eq(1).hasClass(_this.toggleClass)) {
                $wrapper.find('.' + _this.navWrapperClass).append(
                    '<div class="mip-st-lazy-tab-nav-toggle-holder"></div>'
                );
            }
            return $wrapper;
        },
        ...
        _setToggerMore = function() {
            ...
            function toggleUp() {
                ...
                $navLayer.hide();
                $mask.hide();
                $('.mip-st-lazy-tab-nav-toggle-btn').css({
        ...
        _setEvents = function() {
            ...
            $.each(_this.navs, function(i, v){
                ...
                $v.on('click', function(){
                    // 把当前tab记录到数据里
                    window.m.currentTab = $.trim($(this).text());
    ...
    var Tab = function(panel, options) {
        options = options || {};
        this.panel = panel;
        this.current = options.current || 0;      // 当前选中的tab
        this.currentClass = options.currentClass || 'c-tab-nav-selected';
        this.navWrapperClass = options.navWrapperClass || 'c-tab-nav';
        this.navClass = options.navClass || 'c-tab-nav-li';
        this.contClass = options.contClass || 'c-tab-content';
        this.viewClass = options.viewClass || 'c-tab-nav-view';
        this.toggleClass = options.toggleClass || 'c-tab-nav-toggle';
        this.layerClass = options.layerClass || 'c-tab-nav-layer';
        this.layerUlClass = options.layerUlClass || 'c-tab-nav-layer-ul';
        ...
        this.scrollSize = options.scrollSize || '-40';        // tab滚动的size
    ...
    $.extend(Tab.prototype, {
        showContent : function(i){
            ...
            var _this = this;
            if(!cont.cached){
                templates.render(cont).then(function (html) {
                    cont.innerHTML = html;
                    cont.cached = true;
                    $(cont).show();
                });
