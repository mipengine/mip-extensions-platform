/**
 * @file mip-yuanxiaoku-fenshuxian 组件
 * @author
 */
...
define(function (require) {
    ...
    customElement.prototype.firstInviewCallback = function () {
        ...
        var category = (customStorage.get('fenShuXianCategory')
            ? customStorage.get('fenShuXianCategory') : getRequest().category);

        // 高校分数线
        var gaoXiaoYear = $element.find('.gaoxiao-l-year').data('year');

        var gaoXiaoLiberalArtsId = '0';
        if (getRequest().gaoXiaoLiberalArtsId) {
            gaoXiaoLiberalArtsId = getRequest().gaoXiaoLiberalArtsId;
            $element.find('.gaoxiao-l-liberal-arts').val(gaoXiaoLiberalArtsId);
        }
        var gaoXiaoCityId;
        var gaoXiaoLevelId;

        // 专业分数线
        var zhuanYeBigClassId;
        var zhuanYeSmallClassId;
        var zhuanYeSmallClassName;
        var smallClassRequest = true;
        var zhuanYeCityId;
        var zhuanYeLiberalArtsId = '0';
        if (getRequest().zhuanYeLiberalArtsId) {
            zhuanYeLiberalArtsId = getRequest().zhuanYeLiberalArtsId;
            $element.find('.zhuanye-l-liberal-arts').val(zhuanYeLiberalArtsId);
        }
        var zhuanYeLevelId;
        var zhuanYeYear = $element.find('.zhuanye-l-year').data('year');

        // 地区批次分数线
        var diQuYear = $element.find('.diqu-l-year').data('year');
        var diQuLiberalArtsId = '0';
        if (getRequest().diQuLiberalArtsId) {
            diQuLiberalArtsId = getRequest().diQuLiberalArtsId;
            $element.find('.diqu-l-liberal-arts').val(diQuLiberalArtsId);
        }
        var diQuCityId;
        var diQuLevelId;
        // console.log(
        //     `gaoXiaoYear: ${gaoXiaoYear}; gaoXiaoLiberalArtsId: ${gaoXiaoLiberalArtsId}; gaoXiaoCityId: ${gaoXiaoCityId}; gaoXiaoCityId: ${gaoXiaoCityId}; zhuanYeBigClassId: ${zhuanYeBigClassId}; zhuanYeSmallClassId: ${zhuanYeSmallClassId}; zhuanYeCityId: ${zhuanYeCityId}; zhuanYeLiberalArtsId: ${zhuanYeLiberalArtsId}; zhuanYeLevelId: ${zhuanYeLevelId}; zhuanYeYear: ${zhuanYeYear}; diQuYear: ${diQuYear}; diQuLiberalArtsId: ${diQuLiberalArtsId}; diQuCityId: ${diQuCityId}; diQuLevelId: ${diQuLevelId};`
        // )
        ...
        $element.find('.gaoxiao-l-year, .zhuanye-l-year, .diqu-l-year')
        .append(
        ...
        loadSelectVal('.gaoxiao-l-year', function (optVal) {
            gaoXiaoYear = optVal;
        });
        if (getRequest().gaoXiaoYear) {
            gaoXiaoYear = getRequest().gaoXiaoYear;
            $element.find('.gaoxiao-l-year').val(gaoXiaoYear);
        }
        loadSelectVal('.zhuanye-l-year', function (optVal) {
            zhuanYeYear = optVal;
        });
        if (getRequest().zhuanYeYear) {
            zhuanYeYear = getRequest().zhuanYeYear;
            $element.find('.zhuanye-l-year').val(zhuanYeYear);
        }
        loadSelectVal('.diqu-l-year', function (optVal) {
            diQuYear = optVal;
        });
        if (getRequest().diQuYear) {
            diQuYear = getRequest().diQuYear;
            $element.find('.diqu-l-year').val(diQuYear);
        }

        // 文理分科
        $element.find('.gaoxiao-l-liberal-arts, .zhuanye-l-liberal-arts, .diqu-l-liberal-arts')
        .html(
            '<option value=\"0\">不限</option>'
            + '<option value=\"1\">文科</option>'
            + '<option value=\"2\">理科</option>'
        );
        loadSelectVal('.gaoxiao-l-liberal-arts', function (optVal) {
            gaoXiaoLiberalArtsId = optVal;
        });
        loadSelectVal('.zhuanye-l-liberal-arts', function (optVal) {
            zhuanYeLiberalArtsId = optVal;
        });
        loadSelectVal('.diqu-l-liberal-arts', function (optVal) {
            diQuLiberalArtsId = optVal;
        });
        ...
        .then(function (res) {
            ...
            $element.find('.gaoxiao-l-city, .zhuanye-l-city, .diqu-l-city')
            .html(
            ...
            gaoXiaoCityId = list[0].ID;
            if (getRequest().gaoXiaoCityId) {
                gaoXiaoCityId = getRequest().gaoXiaoCityId;
                $element.find('.gaoxiao-l-city').val(gaoXiaoCityId);
            }
            zhuanYeCityId = list[0].ID;
            if (getRequest().zhuanYeCityId) {
                zhuanYeCityId = getRequest().zhuanYeCityId;
                $element.find('.zhuanye-l-city').val(zhuanYeCityId);
            }
            diQuCityId = list[0].ID;
            if (getRequest().diQuCityId) {
                diQuCityId = getRequest().diQuCityId;
                $element.find('.diqu-l-city').val(diQuCityId);
            }
        });
        loadSelectVal('.gaoxiao-l-city', function (optVal) {
            gaoXiaoCityId = optVal;
        });
        loadSelectVal('.zhuanye-l-city', function (optVal) {
            zhuanYeCityId = optVal;
        });
        loadSelectVal('.diqu-l-city', function (optVal) {
            diQuCityId = optVal;
        });
        ...
        .then(function (res) {
            ...
            $element.find('.gaoxiao-l-level, .zhuanye-l-level, .diqu-l-level')
            .html(
                list.map(function (item, index) {
                    return (
            ...
            gaoXiaoLevelId = list[0].ID;
            if (getRequest().gaoXiaoLevelId) {
                gaoXiaoLevelId = getRequest().gaoXiaoLevelId;
                $element.find('.gaoxiao-l-level').val(gaoXiaoLevelId);
            }
            zhuanYeLevelId = list[0].ID;
            if (getRequest().zhuanYeLevelId) {
                zhuanYeLevelId = getRequest().zhuanYeLevelId;
                $element.find('.zhuanye-l-level').val(zhuanYeLevelId);
            }
            diQuLevelId = list[0].ID;
            if (getRequest().diQuLevelId) {
                diQuLevelId = getRequest().diQuLevelId;
                $element.find('.diqu-l-level').val(diQuLevelId);
            }
        });
        loadSelectVal('.gaoxiao-l-level', function (optVal) {
            gaoXiaoLevelId = optVal;
        });
        loadSelectVal('.zhuanye-l-level', function (optVal) {
            zhuanYeLevelId = optVal;
        });
        loadSelectVal('.diqu-l-level', function (optVal) {
            diQuLevelId = optVal;
        });
        ...
        .then(function (res) {
            ...
            $element.find('.zhuanye-l-big-class')
            .html(
                list.map(function (item, index) {
                    return (
                        '<option value=\"' + item.ID + '\">' + item.Name + '</option>'
            ...
            zhuanYeBigClassId = list[0].ID;
            if (getRequest().zhuanYeBigClassId) {
                zhuanYeBigClassId = getRequest().zhuanYeBigClassId;
                $element.find('.zhuanye-l-big-class').val(zhuanYeBigClassId);
            }
            loadMajorSsmallClass();
        });
        loadSelectVal('.zhuanye-l-big-class', function (optVal) {
            zhuanYeBigClassId = optVal;
            loadMajorSsmallClass();
        });
        loadSelectVal('.zhuanye-l-small-class', function (optVal) {
            zhuanYeSmallClassId = optVal;
            zhuanYeSmallClassName = urlEncode($element.find('.zhuanye-l-small-class').find('option').not(
                function () {
                    return !this.selected;
                }
            ).html());
        ...
        function loadMajorSsmallClass() {
            ...
            .then(function (res) {
                ...
                $element.find('.zhuanye-l-small-class')
                .html(
                    list.map(function (item, index) {
                        return (
                ...
                zhuanYeSmallClassId = list[0].ID;
                zhuanYeSmallClassName = urlEncode(list[0].specialname);
                if (getRequest().zhuanYeSmallClassId && smallClassRequest) {
                    smallClassRequest = false;
                    zhuanYeSmallClassId = getRequest().zhuanYeSmallClassId;
                    $element.find('.zhuanye-l-small-class').val(zhuanYeSmallClassId);
                    zhuanYeSmallClassName = urlEncode(
                        $element.find('.zhuanye-l-small-class').find('option').not(
                            function () {
                                return !this.selected;
                            }
                        ).html()
        ...
        // 高校
        var gaoXiaoPage = 1;
        var gaoXiaoCurrentPage = 1;
        var gaoXiaoPageCount = 1;

        var loadGaoXiaoInfo = true;
        var whileLoadGaoXiao = setInterval(function () {
            if (gaoXiaoYear && gaoXiaoLiberalArtsId && gaoXiaoCityId && gaoXiaoLevelId) {
                loadGaoXiao('https://data.api.ppkao.com/Interface/YXK/AcademyMajorApi.ashx'
                + (gaoXiaoLiberalArtsId === '0' ? '' : '&wl=' + gaoXiaoLiberalArtsId), {
                    action: 'GetSchoolMajorLine',
                    Year: gaoXiaoYear,
                    province: gaoXiaoCityId,
                    pcid: gaoXiaoLevelId,
                    page: gaoXiaoPage
                });

                clearInterval(whileLoadGaoXiao);
                // console.log('加载高校分数线')
            }
        }, 300);
        function loadGaoXiao(url, argu) {
            $element.find('.content-gaoxiao .load-info .none').hide();
            $element.find('.content-gaoxiao .load-info .loading').show();
            ...
            .then(function (res) {
                ...
                if (list.S === '0') {
                    $element.find('.content-gaoxiao .load-info .loading').hide();
                    $element.find('.content-gaoxiao .load-info .none').show();
                    return;
                }

                var schoolList = decode(json).sList;

                gaoXiaoCurrentPage = parseInt(list.CurrentPage, 10);
                gaoXiaoPageCount = parseInt(list.PageCount, 10);
                // console.log(gaoXiaoCurrentPage, gaoXiaoPageCount)
                $element.find('.gaoxiao-l-search-result ul.result').append(
                    schoolList.map(function (item, index) {
                        return (
                            ...
                            +            '</span>'
                            +            '<span>'
                            +                '最高分：<b>' + (item.maxfs !== '0' ? item.maxfs : '--') + '</b>'
                            +            '</span>'
                            +            '<span>'
                            +                '平均分：<b>' + (item.varfs !== '0' ? item.varfs : '--') + '</b>'
                            +            '</span>'
                            +            '<span>'
                            +                '最低分：<b>' + (item.minfs !== '0' ? item.minfs : '--') + '</b>'
                            +            '</span>'
                            +        '</article>'
                            +    '</div>'
                            + '</li>'
                ...
                if (gaoXiaoCurrentPage === gaoXiaoPageCount) {
                    $element.find('.content-gaoxiao .load-info .none').show();
                    return;
                }

                loadGaoXiaoInfo = true;
            });
        }
        // 页面 scroll 事件
        $element.find('.details-container .content-gaoxiao').on('scroll', function () {
            var height = $(this).height();
            var scrTop = $(this).scrollTop();
            var scrHeight = $(this)[0].scrollHeight;
            if (height >= scrHeight - scrTop - 100) {
                if (gaoXiaoCurrentPage < gaoXiaoPageCount && loadGaoXiaoInfo) {
                    loadGaoXiaoInfo = false;
                    gaoXiaoPage++;
                    loadGaoXiao('https://data.api.ppkao.com/Interface/YXK/AcademyMajorApi.ashx'
                    + (gaoXiaoLiberalArtsId === '0' ? '' : '&wl=' + gaoXiaoLiberalArtsId), {
                        action: 'GetSchoolMajorLine',
        ...
        $element.find('.details-container .gaoxiao-l-search-result').on('click', 'ul.result li h2', function () {
        ...
        // 专业
        var zhuanYePage = 1;
        var zhuanYeCurrentPage = 1;
        var zhuanYePageCount = 1;

        var loadZhuanYeInfo = true;
        var whileLoadZhuanYe = setInterval(function () {
            if (zhuanYeBigClassId && zhuanYeSmallClassId
                && zhuanYeSmallClassName && zhuanYeCityId
                && zhuanYeLiberalArtsId && zhuanYeLevelId && zhuanYeYear) {
                loadZhuanYe('https://data.api.ppkao.com/Interface/YXK/AcademyMajorApi.ashx'
                + (zhuanYeLiberalArtsId === '0' ? '' : '&wl=' + gaoXiaoLiberalArtsId), {
                    action: 'GetSchoolMajorLine',
                    Year: zhuanYeYear,
                    province: zhuanYeCityId,
                    pcid: zhuanYeLevelId,
                    page: zhuanYePage,
                    sname: zhuanYeSmallClassName
                });

                clearInterval(whileLoadZhuanYe);
                // console.log('加载专业分数线')
            }
        }, 300);
        function loadZhuanYe(url, argu) {
            $element.find('.content-zhuanye .load-info .none').hide();
            $element.find('.content-zhuanye .load-info .loading').show();
            ...
            .then(function (res) {
                ...
                if (list.S === '0') {
                    $element.find('.content-zhuanye .load-info .loading').hide();
                    $element.find('.content-zhuanye .load-info .none').show();
                    return;
                }

                var schoolList = decode(json).sList;

                zhuanYeCurrentPage = parseInt(list.CurrentPage, 10);
                zhuanYePageCount = parseInt(list.PageCount, 10);
                // console.log(zhuanYeCurrentPage, zhuanYePageCount);
                $element.find('.zhuanye-l-search-result ul.result').append(
                    schoolList.map(function (item, index) {
                        return (
                            '<li>'
                            +    '<h2>'
                ...
                if (zhuanYeCurrentPage === zhuanYePageCount) {
                    $element.find('.content-zhuanye .load-info .none').show();
                    return;
                }

                loadZhuanYeInfo = true;
            });
        }
        // 页面 scroll 事件
        $element.find('.details-container .content-zhuanye').on('scroll', function () {
            var height = $(this).height();
            var scrTop = $(this).scrollTop();
            var scrHeight = $(this)[0].scrollHeight;
            if (height >= scrHeight - scrTop - 100) {
                if (zhuanYeCurrentPage < zhuanYeCurrentPage && loadZhuanYeInfo) {
                    loadZhuanYeInfo = false;
                    zhuanYePage++;
                    loadZhuanYe('https://data.api.ppkao.com/Interface/YXK/AcademyMajorApi.ashx'
                    + (zhuanYeLiberalArtsId === '0' ? '' : '&wl=' + gaoXiaoLiberalArtsId), {
                        action: 'GetSchoolMajorLine',
        ...
        $element.find('.details-container .zhuanye-l-search-result').on('click', 'ul.result li h2', function () {
            $(this).find('p').toggleClass('active');
            $(this).parent().find('.details').toggle();
        });
        ...
        loadDiQu();
        function loadDiQu() {
            $element.find('.content-diqu .load-info .none').hide();
            $element.find('.content-diqu .load-info .loading').show();
            ...
            .then(function (res) {
                ...
                $element.find('.content-diqu .diqu-l-search-result ul').html(
                    list.map(function (item, index) {
                        return (
                            '<li data-city-id=\"' + item.ID + '\">'
                            +    '<h2>' + item.ProvinceName + '</h2>'
                            +    '<span>查看详情</span>'
                            + '</li>'
                        );
                    })
                ...
                $element.find('.content-diqu .load-info .loading').hide();
                $element.find('.content-diqu .load-info .none').show();
            });
        }
        $element.find('.content-diqu .diqu-l-search-result').on('click', 'ul li', function () {
            diQuCityId = $(this).data('city-id');
            // console.log(diQuCityId);
            window.location.href = 'diqufenshuxian.html?diQuYear=' + diQuYear
            + '&diQuLiberalArtsId=' + diQuLiberalArtsId + '&diQuCityId=' + diQuCityId
            + '&diQuLevelId=' + diQuLevelId;
        });
        this.addEventAction('load', function (event, str) {
            // console.log(str);
            switch (str) {
                case 'gaoxiao-l-query':
                    // console.log(
                    //     `gaoXiaoYear: ${gaoXiaoYear}; gaoXiaoLiberalArtsId: ${gaoXiaoLiberalArtsId}; gaoXiaoCityId: ${gaoXiaoCityId}; gaoXiaoCityId: ${gaoXiaoCityId};`
                    // )
                    gaoXiaoPage = 1;
                    $element.find('.gaoxiao-l-search-result ul.result').html('');
                    loadGaoXiao('https://data.api.ppkao.com/Interface/YXK/AcademyMajorApi.ashx'
                    + (gaoXiaoLiberalArtsId === '0' ? '' : '&wl=' + gaoXiaoLiberalArtsId), {
                        action: 'GetSchoolMajorLine',
                        Year: gaoXiaoYear,
                    ...
                    break;
                case 'zhuanye-l-query':
                    // console.log(
                    //     `zhuanYeBigClassId: ${zhuanYeBigClassId}; zhuanYeSmallClassId: ${zhuanYeSmallClassId}; zhuanYeSmallClassName: ${zhuanYeSmallClassName}; zhuanYeCityId: ${zhuanYeCityId}; zhuanYeLiberalArtsId: ${zhuanYeLiberalArtsId}; zhuanYeLevelId: ${zhuanYeLevelId}; zhuanYeYear: ${zhuanYeYear};`
                    // )
                    zhuanYePage = 1;
                    $element.find('.zhuanye-l-search-result ul.result').html('');
                    loadZhuanYe('https://data.api.ppkao.com/Interface/YXK/AcademyMajorApi.ashx'
                    + (zhuanYeLiberalArtsId === '0' ? '' : '&wl=' + zhuanYeLiberalArtsId), {
                        action: 'GetSchoolMajorLine',
                        Year: zhuanYeYear,
                        province: zhuanYeCityId,
                        pcid: zhuanYeLevelId,
                ...
                case 'diqu-l-query':
                    // console.log(
                    //     `diQuYear: ${diQuYear}; diQuLiberalArtsId: ${diQuLiberalArtsId}; diQuCityId: ${diQuCityId}; diQuLevelId: ${diQuLevelId};`
                    // );
                    window.location.href = 'diqufenshuxian.html?diQuYear=' + diQuYear
                    + '&diQuLiberalArtsId=' + diQuLiberalArtsId + '&diQuCityId=' + diQuCityId
                    + '&diQuLevelId=' + diQuLevelId;
        ...
        // 高校分数线
        function gaoXiao() {
            $element.find('.details-container .title ul .title-gaoxiao')
            .addClass('current').siblings('li').removeClass('current');
            $element.find('.details-container .content')
            .css({transform: 'translateX(0)'}).addClass('transition');
        }

        // 专业分数线
        function zhuanYe() {
            $element.find('.details-container .title ul .title-zhuanye')
            .addClass('current').siblings('li').removeClass('current');
            $element.find('.details-container .content').css({transform: 'translateX(-33.333%)'});
        }

        // 地区批次分数线
        function diQu() {
            $element.find('.details-container .title ul .title-diqu')
            .addClass('current').siblings('li').removeClass('current');
            $element.find('.details-container .content').css({transform: 'translateX(-66.666%)'});
        }

        // 单击切换导航和内容
        $element.on('click', '.details-container .title ul .title-gaoxiao', function () {
            customStorage.set('fenShuXianCategory', 'gaoxiao');
            $(this).addClass('current').siblings('li').removeClass('current');
            $element.find('.details-container .content')
            .css({transform: 'translateX(0)'}).addClass('transition');
        });
        $element.on('click', '.details-container .title ul .title-zhuanye', function () {
            customStorage.set('fenShuXianCategory', 'zhuanye');
            $(this).addClass('current').siblings('li').removeClass('current');
            $element.find('.details-container .content')
            .css({transform: 'translateX(-33.333%)'}).addClass('transition');
        });
        $element.on('click', '.details-container .title ul .title-diqu', function () {
            customStorage.set('fenShuXianCategory', 'diqu');
            $(this).addClass('current').siblings('li').removeClass('current');
            $element.find('.details-container .content')
            .css({transform: 'translateX(-66.666%)'}).addClass('transition');
        });
