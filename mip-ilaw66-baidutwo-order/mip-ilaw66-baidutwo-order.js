/**
 * @file mip-ilaw66-baidutwo-order 组件
 * @author
 */
                ...
                success: function (data) {
                    ...
                    function evaluateRating(rating) {
                        ...
                        if (data.unpaidAmount === 0) {
                            if (data.commentId === null || data.commentId === '') { // 未评价
                            }
                            else {
                                ratingHtml += '<p class="tilmabox"> <span class="title">服务评价：</span> <span>';
                                for (var i = 0; i < rating; i++) {
                                    ratingHtml += '<mip-img src="tempbaidu/images/icon_star_c_c.png"'
                                        + ' class="starcc" alt="1" title="优"></mip-img>';
                                }
                                ratingHtml += '</span> <p class="pal14rem">这里是用户评价的详情内容，这里是用户评价的详情内容</p></p>';
                            }
                        }
                    ...
                    function btnStatus(unpaidAmount) {
                        if (unpaidAmount === 0) {
                            ...
                            //                  }
                            if (data.commentId === null || data.commentId === '') {
                                return '<button class="toComment orderlist-layout__comment right_btn" id="toComment">'
                                + '去评价</button>';
                            }
                    ...
                    var orderInfoHtml = '<div class="orderlist__status border_bottom">'
                        ...
                        + evaluateRating(2)
                        //                      + '<p class="tilmabox"> <span class="title">服务评价：</span> <span> '+ evaluateRating(2)+' </span> <p class="pal14rem">这里是用户评价的详情内容，这里是用户评价的详情内容</p></p>'
                        + '</div>'
                        + '</div>';
                    ...
                    var lawyerInfoHtml = '<div class="head_portrait">'
                        ...
                        + lawyerOffice + '</div>'
                        + '<div class="con_right"><button '
                        + ' class="toArk orderlist-layout__reAsk reAsk right_btn" data-type="02" lawyerId="'
                        + data.lawyerId + '" questiontype="' + questionType + '">继续问</button></div>';
                    ...
                    $el.find('.sumbtj').click(function () {
                        var requestId = $el.find('#requestId').val();
                        var questionType = $el.find('#questionType').val();
                        window.top.location.href = 'mipilaw66baidu_lawService?requestId='
                        + requestId + '&questionType=' + questionType;
                    });
                    // 希望重试
