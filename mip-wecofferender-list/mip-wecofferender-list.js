/**
 * @file mip-wecofferender-list 组件
 * @author susc
    ...
    customElement.prototype.attributeChangedCallback = function (attributeName, oldValue, newValue, namespace) {
        setTimeout(function () {
            if (attributeName === 'src' && oldValue !== newValue) {
                ...
                viewer.eventAction.execute('refresh', this.element, {
                    e: this.element
                });
            }
    ...
    function refreshContent() {
        ...
        if (element.hasAttribute('preLoad')) {
            ...
            fetchJsonp(url, {
                ...
                if (!data.status && data.data) {
                    // 缓存咖啡列表，便于订单页面的list展现
                    storage.set(coffeeListKey, JSON.stringify(data.data.items));
                    renderTemplate.call(self, data.data);
                }
