/**
 * @file mip-linkeddb-mapupgrade 组件
 * @author
 */
    ...
    customElement.prototype.firstInviewCallback = function () {
        ...
        function MakeSvgPicClass(id, configPar) {
            ...
            this.SVG = d3.select('#' + id).append('svg')
                ...
                .call(d3.zoom().scaleExtent(this.config.scaleExtent).on('zoom', function () {
                    if (That.config.isScale) {
                        That.relMapG.attr('transform', d3.event.transform);
                    }
                }))
            ...
            // 3.4 名字
            this.patterns.append('rect').attr('x', '0')
                .attr('y', 4 / 3 * this.config.r).attr('width', 2 * this.config.r)
                .attr('height', 2 / 3 * this.config.r).attr('fill', 'black').attr('opacity', '0.5');
            ...
            // 4.放关系图的容器
            this.relMapG = this.SVG.append('g')
                .attr('class', 'relMapG')
                .attr('width', this.config.width)
                .attr('height', this.config.height);
            ...
            this.edges = this.relMapG
                ...
                .on('click', function (d) {
                    ...
                    // 浮窗展示
                    var pageX = d3.event.pageX ? d3.event.pageX : (d3.event.clientX
                        + (document.body.scrollLeft || document.documentElement.scrollLeft));
                    var pageY = d3.event.pageY ? d3.event.pageY : (d3.event.clientY
                    ...
                    var Html = '<a class="external linkname" data-no-cache="true" href="'
                    ...
                    if (document.body.clientWidth - pageX < Width) {
                        pageX = document.body.clientWidth - Width - 5;
                    }
            ...
            this.links = this.edges.append('path').attr('class', 'links')
                .attr('d', function (d) { return 'M' + that.R + ',' + 0 + ' L' + getDis(d.source, d.target) + ',0'; })
                .style('marker-end', 'url(#marker)')
                // .attr('refX',this.config.r)
                .attr('stroke', function (d) {
                    var str = d.color ? '#' + d.color : that.config.linkColor;
                    return str;
                });

            // 5.3 添加关系文字的容器
            this.rectG = this.edges.append('g').attr('class', 'rectG');

            // 5.4 添加rect
            var that = this;
            this.rects = this.rectG.append('rect')
                .attr('x', 40)
                .attr('y', -10)
                .attr('width', 40)
                .attr('height', 20)
            ...
            this.texts = this.rectG.append('text')
                .attr('x', 40)
                .attr('y', 5)
                .attr('text-anchor', 'middle')  // <text>文本中轴对齐方式居中  start | middle | end
                .style('font-size', 12).text(function (d) { return d.type; });


            // 6.关系图添加用于显示头像的节点
            this.circles = this.relMapG.selectAll('circle.circleclass')
                .data(this.config.nodes)
                .enter()
                .append('circle')
            ...
            this.ticked = function () {
                ...
                this.links.attr('d', function (d) {
                    return 'M' + that.R + ',' + 0 + ' L' + getDis(d.source, d.target) + ',0';
                });
            ...
            /**
             *  关于缩放平移踩的大坑
             *  1. 改变需要缩放平移对象的transform来实现 ，本例子是 this.relMapG
             *  2. 但是zoom 事件的监听应该放到整个的svg上 ,本例子是 this.SVG
             *  3. 这样 监听出来的 d3.event.transform 的 transform 数值才是正确的
             *  4. 直接把zoom 事件监听 this.relMapG 得到的 transform 数值会爆炸大，移动非常不平顺，瞬移消失在视野
             *
             *  d3.event.transform{
             *      k:1    // 当前比例尺
             *      x:0    // x 方向移动的大小
            ...
            this.highlightObject = function (obj) {
                if (obj) {
                    ...
                    That.dependsNode = That.dependsNode.concat([objIndex]);
                    That.dependsLinkAndText = That.dependsLinkAndText.concat([objIndex]);
                    That.config.links.forEach(function (lkItem) {
                        if (objIndex === lkItem['source']['index']) {
                            That.dependsNode = That.dependsNode.concat([lkItem.target.index]);
                        } else if (objIndex === lkItem['target']['index']) {
                            That.dependsNode = That.dependsNode.concat([lkItem.source.index]);
                        }
                    });

                    // 隐藏节点
                    That.SVG.selectAll('circle').filter(function (d) {
                        return (That.dependsNode.indexOf(d.index) === -1);
                    }).transition().style('opacity', 0.1);
                    // 隐藏线
                    That.SVG.selectAll('.edge').filter(function (d) {
                        // return true;
                        return ((That.dependsLinkAndText.indexOf(d.source.index) === -1)
                            && (That.dependsLinkAndText.indexOf(d.target.index) === -1));
                    }).transition().style('opacity', 0.1);

                } else {
                    // 取消高亮
                    // 恢复隐藏的线
                    That.SVG.selectAll('circle').filter(function () {
                        return true;
        ...
        function getTransform(source, target, dis) {
            var r;
            if (target.x > source.x) {
                if (target.y > source.y) {
                    r = Math.asin((target.y - source.y) / dis);
                } else {
                    r = Math.asin((source.y - target.y) / dis);
                    r = -r;
                }
            } else {
                if (target.y > source.y) {
