...
define(function () {
    ...
    var Tabs = function(panel, options) {
        ...
        // 动画相关
        this.aniLeftMask = options.aniLeftMask || null;
        this.aniRightMask = options.aniRightMask || null;
        this.aniCalPosClass = options.aniCalPosClass || 'mip-vd-tabs-nav-pos';
    ...
    $.extend(Tabs.prototype, {
        animTab: function (ele) {
            var pos = getPosByClassInEle(ele, this.aniCalPosClass);

            var maskStyle = {
                'transition-property': 'transform',
                'transition-duration': '0.22s'
            };
            var aniLeftMaskStyle = $.extend({
                transform: 'translateX(' + pos.left +'px)',
                '-webkit-transform': 'translateX(' + pos.left +'px)'
            }, maskStyle);
            var aniRightMaskStyle = $.extend({
                transform: 'translateX(' + (pos.left + pos.width) +'px)',
                '-webkit-transform': 'translateX(' + (pos.left + pos.width) +'px)'
            }, maskStyle);

            if (this.last < this.current) {
                aniLeftMaskStyle['transition-timing-function'] = 'cubic-bezier(0.895, 0.03, 0.685, 0.22)';
                aniRightMaskStyle['transition-timing-function'] = 'cubic-bezier(0.165, 0.84, 0.44, 1)';
            }
            else {
                aniLeftMaskStyle['transition-timing-function'] = 'cubic-bezier(0.165, 0.84, 0.44, 1)';
                aniRightMaskStyle['transition-timing-function'] = 'cubic-bezier(0.895, 0.03, 0.685, 0.22)';
            }

            this.aniLeftMask.css(aniLeftMaskStyle);
            this.aniRightMask.css(aniRightMaskStyle);
        },
        animContent: function () {
            var current = this.current;
            $.each(this.conts, function (i, v) {
                $(this).css({
                    left: (i - current) * 100 + '%'
                });
            })
            // this.panel.css('height', $(this.conts[current]).height());
        },
