...
define(function (require) {
    ...
    var ANI_CON_CLS = 'mip-vd-tabs-nav-ani-con';
    var ANI_LEFT_MASK_CLS = 'mip-vd-tabs-nav-ani-left-mask';
    var ANI_RIGHT_MASK_CLS = 'mip-vd-tabs-nav-ani-right-mask';
    var ANI_CAL_POS_CLS = 'mip-vd-tabs-nav-pos';
    /**
     * 渲染
     */
    ...
    function getPosByClassInEle(ele, className) {
        if (ele.hasClass(className)) {
            ...
            for (var i = 0, l = childrens.length; i < l; i++) {
                return getPosByClassInEle($(childrens[i]), className);
            }
        }
    }
    ...
    function generateCommonTab() {
        ...
        // 防止由于子元素absolute父元素高度塌陷
        $el.css('height', window.screen.height);

        // 添加tab头底部横滑动画
        var headerTailCon = $('<div class="' + ANI_CON_CLS +'"></div>');
        var headerTailLeftMask = $('<div class="' + ANI_LEFT_MASK_CLS +'"></div>');
        var headerTailRightMask = $('<div class="' + ANI_RIGHT_MASK_CLS +'"></div>');

        headerTailCon.append(headerTailLeftMask);
        headerTailCon.append(headerTailRightMask);
        $header.append(headerTailCon);

        // 异步fix位置
        setTimeout(function() {
            var pos = getPosByClassInEle($currentTabNav, ANI_CAL_POS_CLS);
            headerTailLeftMask.css('transform', 'translateX(' + pos.left +'px)');
            headerTailLeftMask.css('-webkit-transform', 'translateX(' + pos.left +'px)');
            headerTailRightMask.css('transform', 'translateX(' + (pos.left + pos.width) +'px)');
            headerTailRightMask.css('-webkit-transform', 'translateX(' + (pos.left + pos.width) +'px)');
        }, 0);
        ...
        new Tab($el, {
            ...
            aniLeftMask: headerTailLeftMask,
            aniRightMask: headerTailRightMask,
            aniCalPosClass: ANI_CAL_POS_CLS
        });
    }
