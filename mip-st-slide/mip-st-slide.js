    ...
    var touchSlide = function (a) {
        ...
        var obj = function (str, parEle) {
            ...
            for (var ii = 0; ii < par.length; ii++) {
                ...
                for (var r in retn) {
                    if (par[ii][0] === '#') {
                        Retn.push(document.getElementById(par[ii].replace('#', '')));
                    }
                    else if (par[ii][0] === '.') {
                        var tag = retn[r].getElementsByTagName('*');
                        for (var j = 0; j < tag.length; j++) {
                            var cln = tag[j].className;
                            if (cln && cln.search(new RegExp('\\b' + par[ii].replace('.', '') + '\\b')) !== -1) {
                                Retn.push(tag[j]);
                            }
                        ...
                        var tag = retn[r].getElementsByTagName(par[ii]);
                        for (var j = 0; j < tag.length; j++) {
                            Retn.push(tag[j]);
                        }
                    }
        ...
        var addClass = function (ele, className) {
            if (!ele
                || !className
                || (ele.className && ele.className.search(new RegExp('\\b' + className + '\\b')) !== -1)) {
                return;
            }
        ...
        for (var i = 0; i < tempSize; i++) {
            conBox.children[i].style.cssText = 'display:table-cell;vertical-align:top;width:'
            + slideW
            + 'px';
        }
