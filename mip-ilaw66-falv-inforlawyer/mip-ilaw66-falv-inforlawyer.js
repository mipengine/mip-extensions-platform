/**
 * @file mip-ilaw66-falv-inforlawyer 组件
 * @author
 */
//  informLawyer_failed
define(function (require) {
    var $ = require('jquery');
    // zepto不支持is方法，所以依赖于jquery
    ...
    customElement.prototype.firstInviewCallback = function () {
        ...
        if (secondAskFlg >= '2') {
            ...
            $el.find('.alreadyKnow').click(function () {
                window.top.location.href = 'mip_linking?questionType=' + questionType + '&lawyerName='
        ...
        $el.find('.reAsk').click(function () {
            ...
            $.ajax({
                ...
                url: 'continueAskV3?lawyerId=' + lawyerId + '&questionType='
                    + questionType + '&_csrf=' + $el.find('#_csrf').val() + '&source=1',
                success: function (data) {
                    console.log(data);
                    var id = data.data;
                    if (id !== '') {
        ...
        //  弹窗我知道了--取消弹窗
        $el.find('#knowBtn').on('click', function () {
            $el.find('.popUp_confirm').hide();
        });
        //  咨询其他律师按钮事件
        $el.find('#still_reAsk_btn').on('click', function () {
