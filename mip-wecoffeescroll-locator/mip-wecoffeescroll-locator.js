/**
 * @file mip-wecooffeescroll-locator 组件
 * @author susc
...
define(function (require) {
    ...
    customElement.prototype.scrollHandle = function () {
        ...
        if (this.$section.length) {
            ...
            if (current && self.current !== current) {
                ...
                var $currentAnchor = $element.find('[data-anchor="' + current + '"]');
                $currentAnchor.addClass(className);
                self.current = current;
                var curARect = $currentAnchor[0].getBoundingClientRect();
                self.$slideBox.css({
                    width: curARect.width,
                    height: curARect.height,
                    transform: 'translateY(' + $currentAnchor[0].offsetTop + 'px) '
                });
            }
        }

    };
    customElement.prototype.scrollContentHandle = function () {
        var goodslistTop = $('.goodslist').offset().top;
        var head = $('.rt-head');
        var headHeight = head.height();
        var panel = $('.coffee-panel');
        var panelHeight = panel.height();
        var $rtContent = $('.rt-body-content');
        var initTop = headHeight + panelHeight + 10;
        var $sidebar = $('.sidebar');
        var $content = $('.content');
        // console.log({ 'panelHeight:': panelHeight});
        if (goodslistTop <= headHeight) {
            // console.log('goodslistTop <= headHeight');
        ...
        if (goodslistTop > initTop) {
            // console.log('goodslistTop > initTop');
            $sidebar.css('overflow', 'hidden');
            $content.css({
                overflow: 'hidden',
                height: '65vh'
            });
            $rtContent.css('overflow', 'scroll');
        }
        else {
            console.log({
                goodslistTop: goodslistTop
            });
            $sidebar.css('overflow', 'scroll');
            $content.css({
                overflow: 'scroll',
    ...
    customElement.prototype.build = function () {
        ...
        // 添加滑块
        self.$slideBox = $('<div class="sliderBox"></div>').appendTo($ele);
        // 如果内容为静态则直接获取锚点绑定
        ...
        // $allContent.on('scroll', util.fn.throttle(self.scrollContentHandle.bind(self)));
        $wrapper.on('scroll', util.fn.throttle(self.scrollHandle.bind(self)));
        self.scrollHandle();
        // self.scrollContentHandle();
    };
