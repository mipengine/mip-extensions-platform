...
define(function (require) {
    ...
    customElem.prototype.build = function () {
        try {
            if ($('.yc_user').length !== 0) {
                if ($('#hiddenoriginal').val() === 1) {
                    $('.yc_user').show();
                    rewardlist.original = 1;
                    rewardlist.artid = $('#hiddenartid').val();
                    rewardlist.userid = $('#hiddenuserid').val();
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if (result.status === 1) {
                            ...
                            user1html += '<div class=\'yc_star\'>' + rewardlist.getStarrank(result.degree) + '</div>';
                            user1html += '</div>';
                            ...
                            $('.yc_user_2').html('<p class=\'p1\'>你的赞赏是我坚持原创的动力！'
                                + '</p><a id=\'btnreward\' href=\'javascript:void(0);\' '
                            + 'class=\'yc_z\' onclick=\'rewardlist.showRewardAlert();\'>'
                            + '赞赏</a><div class=\'yc_ulist\' id=\'rewarduserslist\'></div>').show();
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.status === 1 && result.count > 0 && result.userlist.length > 0) {
                        $('#rewarduserslist').html('<span id=\'spantotalrewardcount\'>共'
                        ...
                        for (var i = 0; i < result.userlist.length; i++) {
                            ...
                            $('#spantotalrewardcount').before('<a href=\'' + rewarduserhref + '\' class=\'ara1\' '
