...
define(function (require) {
    ...
    function codeHMback(data, element, type) {
        if (data.status === true) {
            ...
            if (type === 'H') {
                element.querySelector('H_stocke').innerHTML = html;
                // 港股数据添加
                var codelist = '';
                for (var i = 0; i < list.length; i++) {
                    ...
                    element.querySelector('#H_stocke').append(html2);
                }
            } else if (type === 'M') {
                element.querySelector('M_stocke').innerHTML = html;
                // 美股数据添加
                var codelist = '';
                // 需要判断股票代码是否以gb_开头
                for (var i = 0; i < list.length; i++) {
                    ...
                    if ('gb_' === head) {
                        ...
                        html2 += id + '"+ code-type="M" code-count="two"></mip-loadJS-tgb>';
                        element.querySelector('#M_stocke').append(html2);
                    }
                }
