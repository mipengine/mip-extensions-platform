/**
 * @file mip-to8to-report-cookie 获取cookie相关操作
 * @author svenlee
...
define(function (require) {
    ...
    // 跟业务有关
    function initReacteTo() {
        var domain = getDomain();
        var cookieId = cookie.get('to8tocookieid');
        var sessionId = cookie.get('to8tosessionid');
        if (!cookieId) {
            cookieId = createGuid();
        ...
        if (!sessionId) {
            sessionId = 's_' + md5(getCurTimestamp() + randomWord() + cookieId);
            cookie.set('to8tosessionid', sessionId, {path: '/', domain: domain, expires: 18e5});
        }
