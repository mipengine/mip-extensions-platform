...
define(function (require) {
    ...
    function jumpUrl(url) {
        ...
        // 在IOS手百
        if (util.platform.isIos() && navigator.indexOf('baiduboxapp') > -1) {
            // 不在轻框架
            if (navigator.indexOf('light') < 0) {

                window.top.location.href = 'baiduboxapp://v1/easybrowse/open?url='
    ...
    customElement.prototype._bindEvents = function (btn) {
        ...
        btn.addEventListener('click', function () {
            ...
            var url = that._makeUrl(that.config.serverUrl, payData);
            // 按钮loading
