...
define(function (require) {
    ...
    customElem.prototype.build = function () {
        ...
        $('title').text(adata[aid] + '运势详情');
        var imgUrl = $('.astro mip-img').data('url').replace('[aid]', aid);
        $('.astro mip-img').attr('src', imgUrl);
        $('.astro mip-img img').attr('src', imgUrl);
        $('.astro em').html(adata[aid - 1] + '<i></i>');
        $.ajax({
            url: 'https://cache.xzw.com/mip/fortune/0/' + myDates() + '/' + aid + '.js',
            dataType: 'jsonp',
            jsonp: 'callback',
