/**
 * @file mip-pcgroup-commentsimple 组件
 * @author xuhuanlin
 */
...
define(function (require) {
    ...
    var comment = {
            ...
            if (!t.checkLogin(index)) {
                ...
                if (cpost.parentNode === item) {
                    if (obj.innerHTML === '回复') {
                        obj.innerHTML = '收起';
                        t.show(cpost);
                        c.cmtTextarea.placeholder = '回复（' + metaData[1] + '）：';
                    }
                    else {
                        obj.innerHTML = '回复';
                        t.hide(cpost);
        ...
        */
        bindDing: function (obj, index) {
            ...
            fetchJsonp(url, {
                ...
                if (data.code === 1) {
                    html = obj.innerHTML;
                    obj.innerHTML = (parseInt(html, 10) + 1);
                }
                else {
        ...
        */
        bind: function (index) {
            ...
            c.listMore.addEventListener('click', function () {
                c.listMore.classList.add('m-cmt-more-load');
                c.listMore.innerHTML = '加载中...';
                c.pageNo = c.pageNo + 1;
                t.getInfo(index, function (n) {
                    c.listMore.classList.remove('m-cmt-more-load');
                    c.listMore.innerHTML = '加载更多评论';
                });
            });
        ...
        */
        ...
        buildHtml: function (d, index) {
            ...
            for (i = 0; i < n; i++) {
                ...
                html += '   <div class="m-cmt-list-item-th">';
                html += '   <span class="m-cmt-list-item-name">';
                html += '       <img src="' + d[i].face + '" />' + d[i].nickName + '</span>';
                html += '   <a class="m-cmt-list-item-ding" data-id="' + d[i].id + '">' + d[i].support + '</a>';
                html += '   </div>';
                html += '   <div class="m-cmt-list-item-tb">';
                html += '       <div class="m-cmt-list-item-body">' + d[i].content + '</div>';
                if (d[i].replyRef) {
                    html += '<div class="m-cmt-list-item-repeat">' + t.createReplay(d[i].replyRef, index) + '</div>';
                }
                html += '   </div>';
                html += '   <div class="m-cmt-list-item-tf">';
                html += '       <span class="m-cmt-list-item-date">';
                html += t.formateTime(d[i].createTime) + '  &bull;  </span>';
        ...
        */
        createHtml: function (data, index, fn) {
            ...
            var c = t.config[index];
            var n = 0;
            var html = '<div class="m-cmt-head">评论</div>';
            n = d.length;
            if (n < 1) {
            ...
            html += '   </div>';
            html += '</div>';
            html += '<a class="m-cmt-more" href="javascript:">加载更多评论</a>';
        ...
        */
        ...
        /**
        *   格式化时间，默认只格式化当天的时间，如果超过一天的就直接显示完整时间
        *
        *   @param {Date} time 评论时间。
        *   @return {string} 返回格式化后的时间
        *
        */
        formateTime: function (time) {
            var ONEDAY = 24 * 3600 * 1e3; // 返回一天的毫秒数
            var now = this.nowTime;
            var timestp = new Date(time.replace(/-/g, '/'));// 统一格式
            if (!isNaN(+timestp)) {
                if (now - timestp < ONEDAY) {
                    time = Math.floor((now - timestp) / ONEDAY * 24);
                    if (time <= 0) {
                        time = Math.ceil((now - timestp) / ONEDAY * 24 * 60) + '分钟前';
                    }
                    else {
                        time += '小时前';
                    }
        ...
        /**
        *   获取服务器时间，只获取一次，如果成功，就拿到请求头时间，不成功只拿本地时间
        *
        */
        getNowtime: function () {
            var t = this;
    ...
    */
    customElement.prototype.firstInviewCallback = function () {
        ...
        element.classList.add('mip-comment-simple');
        if (!url) {
            return;
        }
