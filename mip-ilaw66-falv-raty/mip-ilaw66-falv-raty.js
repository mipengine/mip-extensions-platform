...
define(function (require) {
    ...
    customElement.prototype.firstInviewCallback = function () {
        ...
        if (window.appBridge && window.appBridge.checkAppFeature('CHANGE_WEBVIEW_TITLE')) {
            window.appBridge.changeWebviewTitle('服务评价');
        }
        // 各项目区分颜色
        if (channel === 'eleme') {
        ...
        $el.find('.btn_conment').bind('touchstart', function () {
            commitCommentMsg();
        });
        ...
        $el.find('.userFeedback_div span').click(function () {
            $(this).toggleClass('userFeedback_select');
        });

        // 是否打官司new
        $el.find('.allow_block').on('click', function () {
            if (!isRecommend) {
