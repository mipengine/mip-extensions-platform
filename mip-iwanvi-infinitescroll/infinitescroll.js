...
define(function (require) {
    ...
    InfiniteScroll.prototype = {
        ...
        funinit: function () {
            ...
            if (me.options.firstResult.length) {
                me.scrollPageCache.content = me.scrollPageCache.content.concat(
                    me.funseparatePage(me.options.firstResult));
                me.options.$result.html(me.funwrapPageParentDom(me.scrollPageCache.content[0], 0));
            ...
            // 如果firstResult不存在,调用funscrollBottomFn方法异步加载首屏数据
            if (!me.options.firstResult.length) {
                me.funscrollBottomFn();
            }
        },
        ...
        funhorizontalHack: function () {
            ...
            if (window.orientation !== undefined) {
                ...
                if (window.orientation === 0 || window.orientation === 180) {
                    ...
                    var jsverticalScreenWidth = verticalScreenWidth * $(window).width();
                    var jsMathmax = Math.max(window.screen.width, window.screen.height);
                    verticalScreenWidth = jsverticalScreenWidth / jsMathmax;
                }
            }
            else {
            ...
            me.options.$wrapper.on('scroll' + me.eventSpace, function () {
                ...
                if (me.currentShowPage !== currentShowPage) {
                    ...
                    if (me.options.limitShowPn) {
                        me.funcycleScrollElement(currentShowPage);
                    }
                }
            });
                    ...
                    function (newResultArr) {
                        ...
                        if (newResultArr === 'NULL') {
                            ...
                            me.scrollPageCache.content
                                = me.scrollPageCache.content.concat(me.funseparatePage(newResultArr));
         ...
         * @param  {integer} pn 页码
         */
        funupdateScrollElement: function (pn) {
            var me = this;

            var jsdomnewPage = $(me.funwrapPageParentDom(
                me.scrollPageCache.content[pn], pn));
            me.options.$result.append(jsdomnewPage);

            // 更新变量
            me.currentLoadPage = pn;
            me.scrollerHeight = me.options.$scroller.height();
            me.scrollPageCache.topPosition.push(jsdomnewPage.position().top);
        },
         ...
         * @param  {integer} pn 页码
         */
        funcycleScrollElement: function (pn) {
            var me = this;

            var recycleClass = 'infinite-recycle';
            var startPage = Math.max(pn - Math.floor((me.options.limitShowPn - 1) / 2), 0);
            // 获取所有结果列表dom
            var jsdomresultElement = me.options.$result.find('.' + me.options.scrollPageClass);
            // 选出当前需要被显示的dom页
            var jsdomshouldShowElement = jsdomresultElement.slice(
                startPage, startPage + me.options.limitShowPn);

            // todo:这里应该还有优化空间
            if (jsdomshouldShowElement.length) {
                // 恢复:在应该被显示的dom中选出所有带回收标记标签的元素执行恢复操作
                jsdomshouldShowElement.each(function () {
                ...
                jsdomresultElement.not(jsdomshouldShowElement).not('.' + recycleClass).each(function () {
         ...
         */
        funseparatePage: function (listArr) {
            var me = this;
         ...
         */
        funwrapPageParentDom: function (html, pn) {
            var me = this;
