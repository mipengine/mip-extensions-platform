...
define(function (require) {
    ...
    customElement.prototype.firstInviewCallback = function () {
        ...
        a = a || {};
        var opts = {
            slideCell: e, //  运行效果主对象，必须用id！，例如 slideCell:"#touchSlide"
            titCell: a.titCell || '.hd li', //  导航对象，当自动分页设为true时为“导航对象包裹层”
            mainCell: a.mainCell || '.bd', //  切换对象包裹层
            effect: e.getAttribute('effect') || 'left', //  效果，支持 left、leftLoop
            autoPlay: e.getAttribute('autoPlay') || false, //  自动播放
            delayTime: e.getAttribute('delayTime') || 200, //  效果持续时间
        ...
        var obj = function (str, parEle) {
            ...
            for (var i = 0; i < par.length; i++) {
                ...
                for (var r in retn) {
                    if (par[i][0] === '#') {
                        reTn.push(document.getElementById(par[i].replace('#', '')));
                    } else if (par[i][0] === '.') {
        ...
        /*字符串转换*/
        var index = parseInt(opts.defaultIndex, 10);
        var delayTime = parseInt(opts.delayTime, 10);
        var interTime = parseInt(opts.interTime, 10);
        ...
        var addClass = function (ele, className) {
            var twk = ele.className && ele.className.search(new RegExp('\\b' + className + '\\b')) !== -1;
            if (!ele || !className || twk) {
                return;
            }
        ...
        var removeClass = function (ele, className) {
            var twki = ele.className && ele.className.search(new RegExp('\\b' + className + '\\b')) === -1;
            if (!ele || !className || twki) {
                return;
            }
        ...
        var translate = function (dist, speed, ele) {
            ...
            var eleTD = ele.webkitTransitionDuration;
            var eleMTD = ele.MozTransitionDuration;
            var eleTD2 = ele.msTransitionDuration;
            var eleOTD = ele.OTransitionDuration;
            var eleD = ele.transitionDuration;
            eleTD = eleMTD = eleTD2 = eleOTD = eleD = speed + 'ms';
