/**
 * @file mip-jt-passport 分享组件
 * @author
 * 说明: ------>表单提交
 */

 /* global registerUtil,validationUtil,org,cngold,passport
 */
define(function (require) {
    ...
    var registerUtil;
    var validationUtil;
    var PASSPORT_DOMAIN = '//passport2.cngold.org';
                            ...
                            $.getJSON(url.SEND_MOBILE, data, function (json) {
                                if (json.flag) {
                                    ...
                                    for (var i = 1; i <= secs; i++) {
                                        setTimeout('validationUtil.update(' + i + ',' + wait + ')', i * 1000);
                                    }
                                    setTimeout('validationUtil.timer()', wait);
                                } else {
                        ...
                        ValidationUtils.prototype.appendOverlay = function () {
                            if (mold !== 1 && mold !== 2) {
                                ...
                                var maskDiv = '<div id="fvmask" style="display: block;">'
                                    ...
                                    + '<span onclick="validationUtil.displayOrHidden(\'\',\'\',\'\');"'
									+ 'class="close close_alidate">'
                                    ...
                                    + 'class="no_bg" type="button" value="提交" id="further_veri_mobile"></div>'
                                    + '</div>'
                                    + '</div>';
                                var bodys = document.body;
                                var commentInsertDiv = $('#formWidth');
...
//                                bodys.appendChild(newDiv);
                                commentInsertDiv.append(newDiv);
...
//        }
//    });
    registerUtil = new RegisterUtils;
...
//    registerUtil = __namespace__.org.cngold.passport.RegisterUtils;
//    validationUtil = __namespace__.org.cngold.passport.ValidationUtils;
    ...
    function checkMobileCode() {
        validationUtil.checkMobileCode();
    }
    ...
    function ct(el) {
        var fun = $(el).attr('data');
        $(el).on('click', '.ct-submit', function () {
            submitForm(fun);
        });
        $(el).on('click', '.img_reload', function () {
            refreshCaptcha();
        });
        $(el).on('click', '#captchaImg', function () {
            refreshCaptcha();
        });
        $(el).on('click', '.close_alidate', function () {
            validationUtil.displayOrHidden('', '', '');
        });
        $(el).on('click', '#further_veri', function () {
            checkPiccode();
        });
        $(el).on('click', '#further_veri_mobile', function () {
            checkMobileCode();
        });
        $(el).on('click', '.clearfix', function () {
            showPop();
        });
        $(el).on('click', '.free-code', function () {
            getMobileAuthCode();
        });
    }
