    ...
    function toHtml(data) {
        var html = data.proData.map(function (value) {
            return (
                ...
                + '        </a>\n'
                + '        <p class="info-title">' + value.u_username + '</p>\n'
                + '        <p class="team-labels">\n'
                + label(value.u_prof)
                + '        </p>\n'
                + '        <span on="tap:consult-lightbox.toggle">\n'
                + '            <a href="javascript:;" on="tap:MIP.setData({href:\'https://m.findlaw.cn/ask/ask.php?uid='
                + value.u_uid + '\',number:' + value.u_mobile + '})" class="team-consult" >免费咨询</a>\n'
                + '        </span>\n'
                + '    </div>\n'
                + '</div>'
            );
